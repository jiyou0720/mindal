...
from backend.maria_models import User, Role, UserRole, NicknameHistory # NicknameHistory 임포트 확인
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            g.user_uid = data.get('user_uid') # user_uid도 g 객체에 추가
            g.email = data.get('email') # NEW: g.email 추가
            ...
            if user:
                ...
                g.user_roles = [] # 사용자를 찾을 수 없으면 역할 없음
...
def register():
    ...
    if not all([username, email, password]): # gender, age, major는 선택 사항일 수 있음
...
def login_user():
    ...
    # db.session.refresh(user, ['roles']) # 이미 user 객체에 roles 관계가 로드되어 있을 수 있음
    user_roles_list = [user_role_obj.name for user_role_obj in user.roles] # user.roles는 Role 객체 리스트

    token = jwt.encode({
        'user_id': user.id,
        'username': user.username,
        'nickname': user.nickname,
        'user_uid': user.user_uid, # user_uid도 토큰에 포함
        'email': user.email, # NEW: 이메일도 토큰에 포함
    ...
    return jsonify({
        'message': '로그인 성공!',
        'access_token': token, # 'token' 대신 'access_token'으로 통일
        'user_id': user.id,
        'username': user.username,
        'nickname': user.nickname,
        'user_uid': user.user_uid,
        'email': user.email, # 프론트엔드에서 바로 사용할 수 있도록 이메일도 반환
        'roles': user_roles_list # 프론트엔드에서 바로 사용할 수 있도록 역할 목록도 반환
    }), 200
...
def get_current_user():
    ...
    # NEW: User 모델의 to_dict 메서드를 사용하여 사용자 정보 반환
...
def update_profile():
    ...
    try:
        db.session.commit()
        # NEW: 업데이트된 사용자 정보를 to_dict()로 반환
...
def reset_password():
    ...
    try:
        # 이 부분은 실제 토큰 검증 로직으로 대체되어야 합니다. (예: JWT 토큰, 유효기간, 사용자 ID 확인)
        # 현재는 임시 구현
        ...
        # 임시: 토큰 검증 없이 바로 비밀번호 변경 (실제 서비스에서는 보안상 위험)
        # 실제로는 토큰에서 user_id를 추출하고 해당 user의 비밀번호를 변경해야 함
        # 여기서는 편의상 특정 사용자(예: admin)의 비밀번호를 변경하는 예시로 가정
        # user = User.query.filter_by(email="admin@example.com").first() # 임시 사용자
        # if not user:
        #     return jsonify({'message': '비밀번호 재설정 실패: 사용자 없음.'}), 400

        # 실제 토큰 기반 비밀번호 재설정 로직을 구현해야 합니다.
        # 예시:
        # user = User.query.filter_by(email="temp@example.com").first() # 실제 토큰에서 이메일/ID 추출
        # if user:
        #     user.set_password(new_password)
        #     db.session.commit()
        #     return jsonify({'message': '비밀번호가 성공적으로 재설정되었습니다.'}), 200
        # else:
        #     return jsonify({'message': '유효하지 않은 토큰 또는 사용자입니다.'}), 400

        return jsonify({'message': '비밀번호 재설정 기능은 아직 준비 중입니다. (토큰 검증 필요)'}), 501 # Not Implemented
    except Exception as e:
