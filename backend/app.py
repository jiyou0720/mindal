...
def create_app(test_config=None):
    ...
    # --- [수정] MongoDB 데이터베이스 이름 추출 및 설정 ---
    db_name = None
    if mongo_url:
        path = urlparse(mongo_url).path
        if path and path != "/":
            db_name = path.strip("/")

    if not db_name:
        # Railway의 기본 동작은 종종 DB 이름을 URI에 포함하지 않으므로, 환경 변수 또는 기본값을 사용합니다.
        db_name = os.environ.get("MONGO_DATABASE", "mindbridge_db") # 기본 DB 이름 설정
        print(f"INFO: MongoDB 데이터베이스 이름을 URI에서 찾을 수 없습니다. 기본값 '{db_name}'을 사용합니다.")
    ...
    app.config["MONGO_DBNAME"] = db_name # 명시적으로 데이터베이스 이름 설정
    ...
    print(f"INFO: MongoDB DBNAME: {app.config.get('MONGO_DBNAME')}") # 설정된 DB 이름 로그 출력
    print(">>> 데이터베이스 설정 완료.")
